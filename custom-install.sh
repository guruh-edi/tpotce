...
Options:
  ...
  -t <type>         Type of installation (required if -s is used):
                      ...
                      z - llm w Beelzebub   (requires -u and -p)
...
if [ ! -f installer/install/tpot.yml ] && [ ! -f tpot.yml ]; then
  ...
  if [ -f "installer/install/tpot.yml" ]; then
...
if [ "$myANSIBLE_TAG" = "Debian" ]; then # Debian 13 - sudo seems to apply stricter settings, we now ask for the become password
...
echo "### LLM Beel(z)ebub - T-Pot LLM with Beelzebub installation."
echo "###                   Uses LLM based honeypots Beelzebub."
echo "###                   Requires Ollama (recommended) or ChatGPT subscription."
echo
while true; do
...
### On HIVE run the tpotce/deploy.sh script to join this SENSOR to the HIVE."
  ...
  z | Z)
    echo
    echo "### Installing T-Pot LLM Beelzebub."
    myTPOT_TYPE="HIVE"
    cp ${HOME}/tpotce/compose/llm-bzb.yml ${HOME}/tpotce/docker-compose.yml
    myINFO=""
    break
    ;;
  esac
...
if [ "${myTPOT_TYPE}" == "HIVE" ]; then # If T-Pot Type is HIVE ask for WebUI username and password
